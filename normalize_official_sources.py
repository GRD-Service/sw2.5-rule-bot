...
PDF_PAGE_MARKER_RE = re.compile(r"^\s*p\.\s*[0-9]+\s*$", re.IGNORECASE)
...
def looks_like_row_start(
    ...
    *,
    ...
    if (
        last_target_page is not None
        and target_page < last_target_page
    ):
        return None
    ...
    return target_page, starred, rest
...
def make_record(
    *,
    ...
    return {
        ...
        "starred": starred,
        "source_pdf_pages": sorted(set(source_pdf_pages)),
        "location": None,
        "before": None,
        "after": None,
        "raw_text": raw_text,
...
def parse_errata_pages(pages: list[dict]) -> tuple[list[dict], list[dict]]:
    records: list[dict] = []
    unparsed: list[dict] = []
    ...
    for page in pages:
        ...
        for line in iter_page_lines(page):
            start = looks_like_row_start(
                line,
                last_target_page=last_target_page,
            )

            if start is not None:
                target_page, starred, rest = start

                flush_current()

                current_target_page = target_page
            ...
            if current_target_page is not None:
                ...
                preamble_pdf_pages.append(pdf_page)
    ...
    if preamble_text:
        unparsed.append(
            {
                "reason": "PREAMBLE_OR_NON_NUMERIC_TARGET",
                "source_pdf_pages": sorted(set(preamble_pdf_pages)),
...
def normalize_document(path: Path) -> dict:
    ...
    if not isinstance(document, dict):
        raise ValueError(f"document must be object: {path}")

    pages = extracted.get("pages")
    if not isinstance(pages, list):
        raise ValueError(f"pages must be array: {path}")

    filename = path.name
    source_type = classify_source(filename)
    source_key = source_key_from_filename(filename)

    base = {
        "version": NORMALIZE_VERSION,
        "normalized_at": utc_now_iso(),
        "source_id": (extracted.get("source") or {}).get("id"),
        "source_name": (extracted.get("source") or {}).get("name"),
        "source_type": source_type,
        "source_key": source_key,
        "target_book": None,
        "source_url": document.get("url"),
        "source_sha256": document.get("sha256"),
        "source_last_modified": document.get("last_modified"),
        "extracted_file": str(path),
        "extract_version": extracted.get("version"),
        "page_count": extracted.get("page_count"),
    }

    if source_type == SOURCE_TYPE_ERRATA:
        records, unparsed = parse_errata_pages(pages)

        base.update(
            {
                "record_count": len(records),
                "unparsed_block_count": len(unparsed),
                "records": records,
    ...
    base.update(
        {
            ...
            "records": [],
            "unparsed_blocks": [],
            "pages": full_text_parts,
            "parse_status": "CLASSIFIED_ONLY",
        }
    )
...
def build_report(
    *,
    ...
    type_counts = Counter(
        result.get("source_type", SOURCE_TYPE_OTHER)
        for result in results
    )

    total_records = sum(
        int(result.get("record_count") or 0)
        for result in results
    )

    total_unparsed = sum(
        int(result.get("unparsed_block_count") or 0)
        for result in results
    )
    ...
    return {
        ...
        "errata_record_count": total_records,
        "unparsed_block_count": total_unparsed,
        "documents": [
            {
                "input_file": result.get("_input_file"),
                "output_file": result.get("_output_file"),
                "source_type": result.get("source_type"),
                "source_key": result.get("source_key"),
                "record_count": result.get("record_count"),
                "unparsed_block_count": result.get("unparsed_block_count"),
            }
            for result in results
        ],
        "errors": errors,
...
def main(
    ...
    for index, path in enumerate(input_files, start=1):
        print(f"[{index}/{len(input_files)}] {path.name}")
        ...
        try:
            ...
            normalized["_output_file"] = str(output_path)
            results.append(normalized)

            print(
                "  "
                f"type={normalized['source_type']} "
                f"records={normalized.get('record_count', 0)} "
                f"unparsed={normalized.get('unparsed_block_count', 0)}"
            )
    ...
    print()
    print(f"Normalization manifest: {report_path}")
    print(f"Documents: {report['document_count']}")
    print(f"Errors: {report['error_count']}")
    print(f"Source types: {report['source_type_counts']}")
    print(f"ERRATA records: {report['errata_record_count']}")
    print(f"Unparsed blocks: {report['unparsed_block_count']}")
