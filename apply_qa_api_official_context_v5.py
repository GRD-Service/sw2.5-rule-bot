...
def main() -> None:
    ...
    old = '''- `GroupSNE公式エラッタ・追加訂正` が含まれる場合、それは対応する書籍本文より新しい公式訂正として優先してください。
...
'''
    ...
    new = '''- `GroupSNE公式エラッタ・追加訂正` が含まれる場合、それは対応する書籍本文より新しい公式訂正として優先してください。
- 公式訂正が `replace` の場合は訂正後を採用し、`delete` の場合は削除対象を現行ルールとして扱わず、`append` の場合は追加内容を現行ルールへ加えてください。
- `delete` は「情報が見つからない」という意味ではありません。「公式に削除された」という確定情報として回答してください。
- 公式訂正ブロックに「対応する原本引用ID」がある場合、訂正内容について回答するときはその引用IDを根拠として使用してください。
...
- 回答中で「一覧」「該当するもの」「少なくとも次の～」のように複数対象を列挙する場合、`GroupSNE公式エラッタ・追加訂正` に含まれる直接該当項目をすべて列挙してください。
'''
    ...
    old2 = '''    if not official_context:
        official_context_section = ""
    else:
        official_context_section = (
            "\\n\\n=== GroupSNE公式エラッタ・追加訂正 ===\\n"
...
'''
    ...
    new2 = '''    if not official_context:
        official_context_section = ""
    else:
        official_count = len(official_context_items)
        official_context_section = (
            "\\n\\n=== GroupSNE公式エラッタ・追加訂正 ===\\n"
            f"この質問に直接関連する公式訂正候補は {official_count} 件あります。\\n"
            "以下の公式訂正は、対応する原本記述より優先してください。\\n"
